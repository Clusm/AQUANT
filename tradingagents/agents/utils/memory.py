...
# rewrite paths concurrently; without a lock, interleaved writes corrupt
# the markdown (one entry's text bleeding into another's tag line).
# A module-level Lock is sufficient because all threads live in the same
# process (web daemon threads or propagate_batch). Cross-process safety
# would need a real file lock; that's not the batch-mode failure mode.
...
class TradingMemoryLog:
    ...
    def store_decision(
        ...
        with _MEMORY_LOG_LOCK:
            # 幂等检查与写入同锁,避免并发同 (date,ticker) 双写重复 pending。
            # 快速 raw-text 扫描而非完整解析。
    ...
    def update_with_outcome(
        ...
        """Replace pending tag and append REFLECTION section (single-entry batch)."""
        if not self._log_path or not self._log_path.exists():
            return
